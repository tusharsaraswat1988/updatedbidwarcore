...
export function shouldAcceptMonotonicVersion(cachedVersion, incomingVersion) {
    if (incomingVersion == null || incomingVersion <= 0)
        return true;
    return incomingVersion >= cachedVersion;
}
export function nextMonotonicVersion(cachedVersion, incomingVersion) {
    if (incomingVersion == null || incomingVersion <= 0)
 ...
 */
export function shouldApplyBidDelta(base, incoming) {
    const incomingBid = incoming.currentBid;
    if (incomingBid == null)
        return true;
    const existingBid = base?.currentBid;
    if (existingBid == null || typeof existingBid !== "number")
        return true;
...
export function isBidAckPayload(result) {
    if (result.bidAck === true)
        return true;
 ...
 */
export function decideBidMutationApply(cachedVersion, result) {
...
export function mergeBidFields(prev, incoming, version) {
    const base = (prev ?? {});
    const next = {
        ...base,
    ...
    if (version != null)
        next.eventVersion = version;
    return next;
}
/** Structured bid-lifecycle logger (safe for browser + node). */
export function logBidLifecycle(entry) {
    const payload = {
        scope: "bid-lifecycle",
 ...
 */
export function reduceBidUiPhase(phase, event) {
    switch (event.type) {
        case "submit":
...
export function isBidUiBusy(phase) {
 ...
 */
export function simulateRapidBidVersionRace(steps) {
    let version = 0;
