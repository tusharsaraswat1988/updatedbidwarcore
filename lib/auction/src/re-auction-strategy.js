...
export function parseReAuctionStrategy(raw) {
    if (!raw?.trim())
        return null;
    try {
        ...
        if (mode !== "keep_existing" && mode !== "reset_defaults" && mode !== "fixed")
            return null;
        const fixedAmount = parsed.fixedAmount;
        if (mode === "fixed") {
...
export function serializeReAuctionStrategy(strategy) {
 ...
 */
export function resolveOpeningBid(input) {
    const categoryBase = input.categoryMinBid != null && input.categoryMinBid > 0 ? input.categoryMinBid : null;
...
export function validateFixedReAuctionAmount(amount, tournamentMinBid) {
