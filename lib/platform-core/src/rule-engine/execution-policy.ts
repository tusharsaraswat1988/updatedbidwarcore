...
import type { ConcreteRuleValue, ExecutableRule, ResolvedRuntimeRules } from "./types.ts";
...
function num(rules: readonly ExecutableRule[], id: string, fallback: number): number {
  const v = ruleValue(rules, id);
  return typeof v === "number" ? v : fallback;
}

function bool(rules: readonly ExecutableRule[], id: string, fallback: boolean): boolean {
  const v = ruleValue(rules, id);
  return typeof v === "boolean" ? v : fallback;
}

function str(rules: readonly ExecutableRule[], id: string, fallback: string): string {
  const v = ruleValue(rules, id);
  return typeof v === "string" ? v : fallback;
}

function nullableNum(rules: readonly ExecutableRule[], id: string): number | null {
  const v = ruleValue(rules, id);
