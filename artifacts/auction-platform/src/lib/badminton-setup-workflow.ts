...
export const BADMINTON_SETUP_STEPS: BadmintonSetupStep[] = [
  ...
  },
  {
    ...
    why: "Each court may have a Scorer, Display, and Broadcast so match day runs without confusion.",
  ...
  },
  {
    ...
    why: "Generate Automatically or Create Manually creates planned match-ups for knockout events.",
    after: "After this step you will build the Match Schedule — where and when each fixture is played.",
    examples: ["Generate Automatically", "Create Manually"],
    helperText: "Knockout draws only in this release. Import and round-robin are not available yet.",
  ...
  },
  {
    ...
    purpose: "Match Schedule assigns each fixture to a court and time (setup only — it does not start scoring).",
    what: "Match Schedule assigns each fixture to a court and time. It schedules fixtures, not live Matches.",
    why: "Scheduling happens AFTER fixtures — first you know who plays whom, then you place those fixtures on courts and times. To start scoring, create a Match from a scheduled fixture, then open Match Control.",
    after: "Next: Operations → Matches (create match) → Match Control (start). Scorer Home only shows matches that already have a court and time.",
    examples: ["Court 1 · 10:00 AM", "Court 2 · 10:30 AM"],
    helperText: "This page schedules fixtures. Start matches from Operations → Matches → Match Control.",
  ...
  },
  {
    ...
    after: "Open Matches to create scoring matches, then Match Control to start — or use the Operator Panel.",
    helperText: "Next: Matches → Match Control to start, or Operator Panel for live courts.",
