...
describe("player registration mode", () => {
  ...
  it("sends explicit null for Assign later and omits category when hidden", () => {
    ...
    expect(publicRegistrationCategoryIdPayload(true, "none")).toBeNull();
    expect(publicRegistrationCategoryIdPayload(true, "7")).toBe(7);
  });

  it("coerces scoring-mode sold/retained assignment to available", () => {
    expect(coerceScoringModePlayerStatus("sold", "available")).toBe("available");
    expect(coerceScoringModePlayerStatus("retained", "available")).toBe("available");
    expect(coerceScoringModePlayerStatus("unsold", "available")).toBe("available");
    expect(coerceScoringModePlayerStatus(undefined, "sold")).toBe("available");
    expect(coerceScoringModePlayerStatus("available", "available")).toBe("available");
  });

  it("uses transfer PTA for scoring team assignment and sold for auction", () => {
