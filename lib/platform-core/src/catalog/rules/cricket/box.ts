...
export const CRICKET_BOX_RULE_PROFILES = [
  ruleProfile({
    id: "cricket.box.bpl_kids",
    sportId: "cricket",
    displayName: "BPL Kids Box Cricket",
    description: "Configurable box cricket profile for BPL-style kids matches.",
    ...
    tags: ["box", "kids", "bpl"],
    values: [
      value("cricket.match.overs_per_innings", 5),
      value("cricket.match.max_wickets", 6),
      value("cricket.match.playing_squad_size", 7),
      value("cricket.match.playing_xi_enforced", true),
      value("cricket.match.bench_size", 1),
      value("cricket.match.balls_per_over", 6),
      value("cricket.match.ball_type", "tennis"),
      value("cricket.dismissal.lbw_enabled", false),
      value("cricket.extras.leg_bye_enabled", false),
      value("cricket.bowling.free_hit_enabled", true),
      value("cricket.batting.retire_at_runs", null),
      value("cricket.powerplay.enabled", true),
      value("cricket.tie_break.ties_allowed", true),
      value("cricket.tie_break.super_over_enabled", true),
      value("cricket.special.super_ball_enabled", true),
      value("cricket.tie_break.super_over_overs", 1),
      value("cricket.tie_break.super_over_wickets", 2),
      value("cricket.tie_break.super_over_trigger", "knockout_tie"),
      value("cricket.boundary.four_runs", 4),
      value("cricket.boundary.six_runs", 6),
    ],
    runtimeBinding: {
      runtimeBindingType: "cricket_platform_defaults",
      runtimeBindingId: "box_bpl_kids",
    },
  }),
  ruleProfile({
  ...
  ruleProfile({
    ...
    description:
      "Deprecated pack retained for compatibility tests — not selectable on create.",
    supportedCompetitionTypes: ALL_COMP,
    supportedVariants: ["cricket.box"],
    status: "deprecated",
    recommendation: "advanced",
