...
export function describeBidIncrementRules(tournament, unit = "rupee") {
    ...
    tiers.forEach((tier, index) => {
        ...
        if (isLast && tier.upTo == null) {
            ...
            if (previousUpTo != null && previousUpTo > 0) {
                lines.push(`Above ${formatShortAuctionAmount(previousUpTo, unit)}: raise by ${increment} or more.`);
            }
            else {
        ...
        if (tier.upTo != null && tier.upTo > 0) {
            if (index === 0) {
                ...
                lines.push(`Above ${formatShortAuctionAmount(previousUpTo, unit)} up to ${formatShortAuctionAmount(tier.upTo, unit)}: raise by ${increment} or more.`);
            }
        }
    });
    return lines;
