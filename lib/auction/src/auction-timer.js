...
export function parseAuctionTimerSeconds(raw) {
    if (raw === "" || raw == null)
        return null;
...
export function validateAuctionTimerSeconds(seconds, label) {
    if (seconds == null) {
...
export function parseOperatorTimerSeconds(raw, fallback) {
    const n = parseInt(raw, 10);
    if (!Number.isFinite(n) || n < MIN_AUCTION_TIMER_SECONDS)
        return fallback;
