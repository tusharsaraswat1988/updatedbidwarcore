...
export function isAuctionActivityEventType(type) {
...
export function recordAuctionActivity(tournamentId, atMs) {
    if (!Number.isFinite(atMs))
        return;
    const prev = activityByTournament.get(tournamentId) ?? 0;
    if (atMs > prev)
        activityByTournament.set(tournamentId, atMs);
}
export function getRecordedAuctionActivity(tournamentId) {
    return activityByTournament.get(tournamentId) ?? null;
}
export function clearRecordedAuctionActivity(tournamentId) {
...
export function resolveAuctionFeedState(input) {
    ...
    const secondsSinceLastActivity = lastActivityAt == null ? null : Math.max(0, Math.floor((now - lastActivityAt) / 1000));
...
export function formatLastActivityDiagnostic(secondsSinceLastActivity) {
    if (secondsSinceLastActivity == null)
        return null;
    return `Last activity ${secondsSinceLastActivity}s ago`;
}
export const INITIAL_CONNECTION_BANNER_PULSE = {
    lastShownAt: null,
    visibleUntil: null,
};
/** Show briefly, hide, then re-show every remind interval while feed stays non-live. */
export function advanceConnectionBannerPulse(feedState, now, pulse) {
    if (feedState === "live") {
    ...
    const due = pulse.lastShownAt == null ||
...
export function nextConnectionBannerPulseDelayMs(feedState, pulse, now) {
    if (feedState === "live")
        return CONNECTION_BANNER_REMIND_INTERVAL_MS;
    if (pulse.visibleUntil != null && now < pulse.visibleUntil) {
        return pulse.visibleUntil - now;
    }
    if (pulse.lastShownAt == null)
        return 0;
