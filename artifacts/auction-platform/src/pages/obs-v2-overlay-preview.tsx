...
import {
  ...
  broadcastOverlayV2Path,
} from "@/lib/broadcast-overlay";

type FacingMode = "user" | "environment";

/**
 * Camera + Broadcast Overlay v2 preview — same stack as v2 Browser Source.
 */
export default function ObsV2OverlayPreview() {
  const [, v2Params] = useRoute("/tournament/:id/obs/v2/preview");
  const [, labParams] = useRoute("/tournament/:id/obs/lab/preview");
  const tournamentId = parseInt(v2Params?.id || labParams?.id || "0", 10);
  ...
  const overlaySrc = broadcastOverlayV2Path(tournamentId);
  ...
  useEffect(() => {
    document.title = "Overlay v2 Preview — BidWar";
