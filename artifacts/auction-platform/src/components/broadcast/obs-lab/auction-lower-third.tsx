...
import { BID_PULSE_DURATION_MS, OBS_BID_PANEL, OBS_LAB_FONTS } from "./obs-tokens";
  ...
  return (
    <div>
      ...
      {animateAmount ? (
        <motion.div
          key={amount}
          initial={{ scale: 0.78, opacity: 0.65, filter: "brightness(1.6)" }}
          animate={{
            scale: [0.78, 1.12, 0.98, 1],
            opacity: 1,
            filter: ["brightness(1.6)", "brightness(1.25)", "brightness(1)"],
          }}
          transition={{ duration: BID_PULSE_DURATION_MS / 1000, ease: [0.22, 1.35, 0.36, 1] }}
          style={{
            fontSize: OBS_BID_PANEL.bidFont,
            fontWeight: 400,
            fontFamily: OBS_LAB_FONTS.display,
            color,
            lineHeight: 1,
            textShadow: `0 2px 14px rgba(0,0,0,0.95), 0 0 28px ${color}88`,
