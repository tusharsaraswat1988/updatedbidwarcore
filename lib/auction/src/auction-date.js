...
export const AUCTION_DATE_PAST_ERROR = "Auction date must be today or a future date.";
...
export function validateAuctionDate(dateStr) {
...
export function parseAuctionDateString(dateStr) {
    const trimmed = dateStr.trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(trimmed))
        return undefined;
